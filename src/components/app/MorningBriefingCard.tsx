...
import { ArrowDownRight, ArrowUpRight, CalendarDays, Coffee, Lightbulb, Loader2, RefreshCw, Sparkles, TriangleAlert } from "lucide-react";
...
export function MorningBriefingCard({ workspaceId, className }: { workspaceId: string; className?: string }) {
  ...
  return (
    <section
      className={cn("surface-ink relative overflow-hidden rounded-2xl border border-foreground/10 p-5 shadow-lift sm:p-6", className)}
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-background/65">إحاطة {eva?.name ?? "أمَل"} اليومية</p>
            <h2 className="mt-1 font-display text-lg font-black leading-snug text-background sm:text-xl">
        ...
        <button
          ...
          disabled={isFetching}
          className="inline-flex min-h-11 items-center gap-1.5 rounded-xl border border-background/20 px-3 py-2 text-xs font-bold text-background transition-colors hover:bg-background/10 disabled:opacity-50"
        >
        ...
        <>
          <div className="mt-5 grid grid-cols-3 gap-2 [&>*]:min-w-0">
            {[
              ...
              <div key={s.k} className="rounded-xl border border-background/10 bg-background/8 px-3 py-3 sm:px-4">
                <p className="truncate text-[10px] text-background/60 sm:text-[11px]">{s.k}</p>
                <p className="font-display text-xl font-black text-background">{s.v.toLocaleString("ar-EG")}</p>
              </div>
            ))}
          </div>
          ...
          {data.attention.length ? (
            <ul className="mt-4 space-y-1.5">
              {data.attention.map((a) => (
                <li key={a} className="flex items-center gap-2 rounded-xl bg-amber/15 px-3 py-2 text-xs font-semibold text-background">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2 [&>*]:min-w-0">
            <div>
              <h3 className="flex items-center gap-1.5 text-sm font-black text-background">
              ...
              {data.todayPosts.length ? (
                <ul className="mt-2 space-y-1.5">
                  {data.todayPosts.map((p) => (
                    <li key={p.id}>
                      <Link to="/app/calendar" className="flex items-center gap-2 rounded-xl border border-background/15 bg-background/6 p-1.5 pe-3 text-xs text-background transition-colors hover:bg-background/12">
                        {p.image ? (
                ...
                <ul className="mt-3 space-y-1">
                  {data.rankMoves.map((m) => (
                    <li key={m.keyword} className="flex items-center justify-between gap-2 text-xs">
                      <span className="truncate text-background">{m.keyword}</span>
            ...
            <div>
              ...
              {data.ideas.length ? (
                  ...
                  {data.ideas.map((i) => (
                    <li key={i.title}>
                      <Link
                        ...
                        className="group block rounded-xl border border-background/15 bg-background/6 px-3 py-2 text-xs text-background transition-colors hover:bg-background/12"
                      >
                        <span className="flex items-center gap-1.5 font-bold">
                          <AppIcon name={i.provider} className="size-3.5" /> {i.title}
                        </span>
                        <span className="mt-0.5 block text-background/55">«{i.hook}»</span>
                ...
                <p className="mt-2 text-xs text-background/60">أفكار اليوم ستظهر بعد تحليل علامتك.</p>
              )}
            </div>
          </div>
        </>
